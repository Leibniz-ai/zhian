    ...
    // ============================================================================
    const DashboardView = ({ activeRegions, alerts, onAlertClick }) => {
      ...
      const filteredAlerts = useMemo(() => {
        ...
        return displayAlerts.filter(alert => {
          const matchSeverity =
            ...
            (alertSeverityFilter === 'p0' && alert.severity === 'P0') ||
            (alertSeverityFilter === 'other' && alert.severity !== 'P0');
          const matchKeyword =
            !keyword ||
            alert.type.toLowerCase().includes(keyword) ||
            alert.location.toLowerCase().includes(keyword) ||
            alert.camera.toLowerCase().includes(keyword) ||
            alert.id.toLowerCase().includes(keyword);
      ...
      return (
        <div className="flex h-full gap-4 relative overflow-hidden animate-in fade-in duration-300">
            ...
            <div className="flex-1 flex flex-col gap-4 z-10 group relative" onWheel={handleWheel}>
              <div className={`bg-[#080d19] border ${activeRegionId ? 'border-cyan-500/60 shadow-[0_0_40px_rgba(34,211,238,0.2)]' : 'border-cyan-500/30 shadow-[0_0_30px_rgba(8,145,178,0.15)]'} rounded-xl flex-1 relative overflow-hidden flex flex-col transition-colors duration-500`}>
                ...
                <div className="absolute top-0 left-0 right-0 h-10 bg-gradient-to-b from-slate-900/95 to-transparent z-30 flex items-center justify-between px-4">
                  ...
                  <div className="flex items-center space-x-3">
                    <span className="bg-slate-900/80 px-2 py-1 rounded border border-slate-700 text-[10px] text-slate-300 flex items-center shadow-sm">
                      ...
                      {!activeRegionId ? '按住拖动平移，向上滚轮放大' : '按住拖动平移，向下滚轮返回全局态势'}
                    </span>
                    {activeRegionId && (
                ...
                {!activeRegionId && displayAlerts.some(a => a.severity === 'P0') && (
                  <div className="absolute z-10 pointer-events-none" style={{ top: '25%', left: '42%' }}> 
            ...
            <div className="w-1/4 flex flex-col gap-4 z-10">
              <div className="bg-slate-900/60 border border-cyan-900/50 rounded-xl p-5 backdrop-blur-sm flex-1 flex flex-col overflow-hidden shadow-lg">
                <div className="flex items-center justify-between mb-3 gap-3">
                  <h2 className="text-sm font-semibold text-cyan-400 flex items-center shrink-0"><Bell className="w-4 h-4 mr-2" />实时智能预警流</h2>
                ...
                {displayAlerts.length === 0 ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-500">
                    ...
                    <span className="text-sm">没有匹配到对应预警</span>
                    <span className="text-[11px] text-slate-600 mt-2">试试更换关键词或筛选条件</span>
                  </div>
                ) : (
                  <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar">
                    {filteredAlerts.map(alert => (
                      <div key={alert.id} onClick={() => onAlertClick(alert)} className={`p-3 rounded-xl border cursor-pointer transition-all group shadow-sm ${alert.severity === 'P0' ? 'bg-red-950/40 border-red-500/60 hover:bg-red-900/60' : 'bg-slate-800/60 border-slate-700 hover:border-cyan-500/50 hover:bg-slate-800'}`}>
                        <div className="flex items-center justify-between mb-2">
                          <span className={`font-bold text-xs ${alert.severity === 'P0' ? 'text-red-400' : 'text-orange-400'}`}>{stripPriorityLabel(alert.type)}</span>
                          <span className="text-slate-500 text-[10px] font-mono group-hover:text-white transition-colors">{alert.time}</span>
                        </div>
                        <div className="text-[11px] text-slate-400 flex items-center justify-between">
                          <span className="flex items-center truncate"><MapPin className="w-3.5 h-3.5 mr-1 shrink-0" />{alert.location}</span>
                          <span className={`text-white px-2 py-0.5 rounded ml-2 shrink-0 text-[10px] font-bold ${alert.severity === 'P0' ? 'bg-red-600 animate-pulse border border-red-400/50' : 'bg-orange-600 border border-orange-400/50'}`}>{severityText(alert.severity)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
    ...
    // 警情复核弹窗 (Alert Modal)
    // ============================================================================
    const AlertVerificationModal = ({ alert, onClose, onResolve }) => {
      ...
      const confirmLabel = isFightReview
        ? '确认属实并派警'
        : isCrowdGathering
        ? '确认聚集风险，立即派单疏导'
        : isCritical
      ...
      return (
        <div className="absolute inset-0 bg-slate-950/85 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className={`bg-slate-900 border-2 ${isCritical ? 'border-red-500 shadow-[0_0_50px_rgba(239,68,68,0.3)]' : 'border-orange-500/50 shadow-[0_0_30px_rgba(249,115,22,0.15)]'} rounded-xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col`}>
            ...
            <div className={`h-12 ${isCritical ? 'bg-red-600' : 'bg-orange-950/80'} border-b border-slate-700 flex items-center justify-between px-5`}>
              <div className="flex items-center space-x-2">
                <AlertOctagon className={`w-5 h-5 ${isCritical ? 'text-white animate-pulse' : 'text-orange-400'}`} />
                <h3 className="text-white font-bold text-base tracking-wide">{isCritical ? '重点警情 / 极速指令下发台' : '线索告警复核台'}：{stripPriorityLabel(alert.type)}</h3>
              </div>
            ...
            <div className="flex h-[480px]">
              <div className="flex-1 bg-black relative flex flex-col border-r border-slate-700">
                <div className={`flex-1 relative flex items-center justify-center overflow-hidden bg-slate-900 img-placeholder-container ${isCrowdGathering ? 'bg-[radial-gradient(circle_at_top,rgba(14,165,233,0.22),transparent_36%),linear-gradient(135deg,#020617_0%,#0f172a_48%,#111827_100%)]' : ''} ${isFightReview ? 'bg-[radial-gradient(circle_at_top,rgba(239,68,68,0.18),transparent_38%),linear-gradient(135deg,#020617_0%,#111827_48%,#1f2937_100%)]' : ''}`}>
                  ...
                  {isFightReview && (
                    <>
                      ...
                      <div className="absolute right-5 bottom-5 w-[260px] bg-slate-950/85 border border-red-500/30 rounded-xl p-3 backdrop-blur-sm shadow-2xl z-20">
                        <div className="text-red-300 text-xs font-bold flex items-center mb-2">
                          <PlayCircle className="w-4 h-4 mr-1.5" /> 短视频回放
                        </div>
                        <div className="text-[11px] text-slate-300 leading-relaxed">{fightWindow.summary || alert.sceneSummary}</div>
                      </div>
                    </>
                  )}
                  {isCrowdGathering && (
                    <>
                      ...
                      <div className="absolute left-[16%] top-[18%] right-[16%] bottom-[18%] border-2 border-red-500/80 bg-red-500/[0.08] shadow-[0_0_35px_rgba(239,68,68,0.25)] z-10">
                        ...
                        <div className="absolute right-3 bottom-3 bg-slate-950/80 border border-cyan-500/30 rounded-lg px-3 py-2 text-[11px] text-cyan-100 shadow-lg max-w-[220px]">
                          <div className="text-cyan-400 font-semibold mb-1 flex items-center">
                            <Users className="w-3.5 h-3.5 mr-1.5" /> AI 场景判断
                          </div>
                          <div className="text-slate-300 leading-relaxed">{alert.sceneSummary}</div>
                        </div>
                      </div>
                      <div className="absolute left-5 bottom-5 right-5 z-20">
                        <div className="bg-slate-950/75 border border-slate-700/80 rounded-xl p-3 backdrop-blur-sm shadow-2xl">
                          <div className="flex items-center justify-between text-[10px] text-slate-400 mb-2">
                            <span className="flex items-center"><PlayCircle className="w-3.5 h-3.5 mr-1 text-cyan-400" />事件片段回传</span>
                            <span>片段时长 {clipRange.pre + clipRange.post}s</span>
                          </div>
                          <div className="w-full h-2 bg-slate-800 rounded-full relative overflow-hidden border border-slate-700">
                            <div className="absolute inset-y-0 left-0 w-[38%] bg-cyan-500/35"></div>
                            <div className="absolute inset-y-0 left-[38%] w-[12%] bg-red-500 shadow-[0_0_12px_rgba(239,68,68,0.95)] animate-pulse"></div>
                            <div className="absolute inset-y-0 left-[50%] right-0 bg-slate-700/70"></div>
                          </div>
                          <div className="mt-2 flex items-center justify-between text-[10px] text-slate-500 font-mono">
                            <span>-{clipRange.pre}s 事前追溯</span>
                            <span className="text-red-400 font-bold">触发点 {alert.time}</span>
                            <span>+{clipRange.post}s 事后跟踪</span>
                          </div>
                        </div>
                      </div>
                    </>
                ...
                <div className="h-16 bg-slate-900 border-t border-slate-800 p-4 flex flex-col justify-center shrink-0">
                  <div className="flex justify-between text-xs text-slate-500 mb-2">
                    <span>{isFightReview ? `-${clipRange.pre}s (事前接触追溯)` : isCrowdGathering ? `-${clipRange.pre}s (事前客流追溯)` : '-30s (事前行为追溯)'}</span><span className={`${isCritical ? 'text-red-400' : 'text-orange-400'} font-bold`}>事件触发点 {alert.time}</span><span>{isFightReview ? `+${clipRange.post}s (事后追打判断)` : isCrowdGathering ? `+${clipRange.post}s (事后疏散判断)` : '+30s (事后流窜判断)'}</span>
                  </div>
              ...
              <div className={`w-[360px] bg-slate-800 flex flex-col p-6 ${isCrowdGathering ? 'bg-[linear-gradient(180deg,rgba(15,23,42,0.98),rgba(30,41,59,0.94))]' : ''} ${isFightReview ? 'bg-[linear-gradient(180deg,rgba(15,23,42,0.98),rgba(31,41,55,0.96))]' : ''}`}>
                <h4 className="text-sm font-bold text-slate-300 border-b border-slate-700 pb-3 mb-4">{isFightReview ? '打架斗殴极速复核台' : isCrowdGathering ? '聚集风险智能处置台' : 'AI 智能辅助研判报告'}</h4>
                <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-1">
                  <div className="flex gap-3">
                    <div className="flex-1 bg-slate-900/50 p-3 rounded-lg border border-slate-700/50">
                      <div className="text-[10px] text-slate-500 mb-1.5 uppercase tracking-wider">系统置信度</div>
                      <div className={`text-2xl font-mono font-bold ${isCritical ? 'text-red-400' : 'text-orange-400'}`}>{alert.conf}</div>
                    </div>
                    <div className="flex-1 bg-slate-900/50 p-3 rounded-lg border border-slate-700/50">
                      <div className="text-[10px] text-slate-500 mb-1.5 uppercase tracking-wider">告警定级</div>
                      <div className={`text-sm font-bold mt-1 px-2.5 py-1 rounded inline-block ${isCritical ? 'bg-red-600 text-white animate-pulse' : 'bg-orange-900/50 text-orange-400 border border-orange-800'}`}>
                        {alert.urgency}
                      </div>
                    </div>
                  </div>
                  <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700/50">
                    <div className="text-[10px] text-slate-500 mb-1.5 uppercase tracking-wider">定位信息</div>
                    <div className="text-sm text-slate-200 flex font-medium"><MapPin className="w-4 h-4 mr-1.5 text-cyan-500 shrink-0" /> {alert.location}</div>
                  </div>
                  {isFightReview && (
                    <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                      <div className="flex items-center text-sm font-bold text-slate-200 mb-3">
                        <Shield className="w-4 h-4 mr-2 text-cyan-400" />现场联动信息
                      </div>
                  ...
                  {isCrowdGathering && gatheringMetrics.length > 0 && (
                    <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                      <div className="flex items-center text-sm font-bold text-cyan-300 mb-3">
                        <BarChart3 className="w-4 h-4 mr-2 text-cyan-400" />量化指标
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        {gatheringMetrics.map(metric => (
                          <div key={metric.label} className="bg-slate-950/70 border border-slate-700/60 rounded-lg p-3">
                            <div className="text-[10px] text-slate-500 mb-1">{metric.label}</div>
                            <div className={`text-base font-mono font-bold ${metric.accent || 'text-slate-200'}`}>{metric.value}</div>
                            <div className="text-[10px] text-slate-500 mt-1 leading-relaxed">{metric.note}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  )}
                  {isCrowdGathering && quickActions.length > 0 && (
                    <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                      <div className="flex items-center text-sm font-bold text-slate-200 mb-3">
                        <Siren className="w-4 h-4 mr-2 text-orange-400" />快捷处置动作
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {quickActions.map(action => (
                          <button key={action} className="px-3 py-2 text-xs font-medium rounded-lg border border-cyan-700/50 bg-cyan-950/30 text-cyan-200 hover:bg-cyan-900/40 transition-colors">
                            {action}
                          </button>
                        ))}
                      </div>
                  ...
                  <div className={`p-4 rounded-xl border ${isCritical ? 'bg-red-950/60 border-red-500/60' : 'bg-orange-950/30 border-orange-500/40'} shadow-inner`}>
                    <div className="flex items-center text-sm font-bold mb-3 pb-3 border-b border-white/10">
                      {isCritical ? <ShieldAlert className="w-4 h-4 mr-2 text-red-400" /> : <Activity className="w-4 h-4 mr-2 text-orange-400" />}
                      <span className={isCritical ? 'text-red-400' : 'text-orange-400'}>系统研判与处置建议 (SOP)</span>
                    </div>
                    {(isCrowdGathering || isFightReview) && sopSteps.length > 0 ? (
                      <div className="space-y-2.5">
                        {sopSteps.map((step, index) => (
                          <div key={step} className="flex items-start">
                            <div className="w-5 h-5 rounded-full bg-red-500/20 border border-red-400/40 text-red-300 text-[11px] font-bold flex items-center justify-center mt-0.5 mr-2 shrink-0">
                              {index + 1}
                            </div>
                ...
                <div className="pt-5 border-t border-slate-700 space-y-3 mt-4">
    ...
    // 系统配置模块
    // ============================================================================
    const SystemConfigView = ({ activeRegions, setActiveRegions }) => {
      ...
      const renderRegionConfig = () => {
        return (
          <div className="flex flex-col h-full animate-in fade-in duration-300">
            <div className="mb-6">
              <h2 className="text-xl font-bold text-slate-100 mb-2">区域配置 (防区管理)</h2>
              <p className="text-sm text-slate-500">从全区海量街道与网格中，勾选需要启用底层AI算法布控的重点业务防区。开启后将同步展示在态势大屏。</p>
            </div>
            ...
            <div className="flex-1 flex gap-6 overflow-hidden">
              <div className="w-1/2 bg-slate-900/40 border border-slate-800 rounded-xl p-5 overflow-y-auto custom-scrollbar">
                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center">
                  <Network className="w-4 h-4 mr-2" /> 全区街道与网格树
                </h3>
                <div className="space-y-4">
                  {REGION_TREE_DATA.map(street => (
                    <div key={street.id} className="border-b border-slate-800/50 pb-3 last:border-0">
                      <div className="flex items-center text-slate-300 font-medium mb-2">
                        {street.isExpanded ? <ArrowDownS className="w-4 h-4 mr-1 text-slate-500" /> : <ArrowRightS className="w-4 h-4 mr-1 text-slate-500" />}
                        <Folder className="w-4 h-4 mr-2 text-cyan-500/70" />
                        {street.name}
                      </div>
                      <div className="pl-7 space-y-1.5">
                        {street.children.filter(c => c.name.includes(searchRegionQuery)).map(region => {
                          const isConfigured = !!ALL_REGIONS_CONFIG[region.id];
                          const isActive = activeRegions.includes(region.id);
                          return (
                            <div key={region.id} className="flex items-center justify-between p-2 rounded hover:bg-slate-800/50 transition-colors">
                              ...
                              {isConfigured ? (
                                ...
                                <span className="text-xs text-slate-600 bg-slate-900 px-2 py-1 rounded border border-slate-800">暂无设备</span>
                              )}
                            </div>
                          )
                        })}
                        ...
                        <div key={id} className="bg-slate-800/60 border border-cyan-900/30 p-4 rounded-lg flex items-center justify-between shadow-sm">
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <div className="text-[10px] text-slate-500 mt-0.5">包含 {r.cameras.length} 路监控通道 | {r.caps.length} 种算法能力</div>
                            </div>
                          </div>
      ...
      const renderCameraConfig = () => {
        ...
        return (
          <div className="flex flex-col h-full min-h-0 animate-in fade-in duration-300">
            <div className="mb-6">
              <h2 className="text-xl font-bold text-slate-100 mb-2">摄像头点位配置</h2>
              <p className="text-sm text-slate-500">将已启用防区下的摄像头点位以示意图、详情和清单联动方式统一管理，便于快速定位、筛选与维护。</p>
            </div>
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4 mb-5 shadow-sm">
              <div className="flex flex-wrap items-end justify-between gap-4">
                  ...
                  <div>
                    ...
                    >
                      {activeRegions.length === 0 ? <option value="">暂无</option> : activeRegions.map(id => (
                        <option key={id} value={id}>{ALL_REGIONS_CONFIG[id].name}</option>
                      ))}
                    </select>
                  </div>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <button type="button" className="bg-cyan-950/50 hover:bg-cyan-900/50 text-cyan-300 text-sm px-4 py-2.5 rounded-lg font-medium border border-cyan-800/50 transition-colors">
                    <Plus className="w-4 h-4 mr-1.5 inline-block" />新增点位
                  </button>
                  <button type="button" className="bg-slate-950 hover:bg-slate-900 text-slate-200 text-sm px-4 py-2.5 rounded-lg font-medium border border-slate-700 transition-colors">
                    保存配置
                  </button>
                </div>
              </div>
              ...
              {currentRegion && (
                <div className="grid grid-cols-4 gap-3 mt-4">
                  <div className="bg-slate-950/60 border border-slate-800 rounded-lg px-4 py-3">
                    <div className="text-[11px] text-slate-500 mb-1">当前防区点位</div>
                    <div className="text-2xl font-bold text-slate-100">{cameraSummary.total}</div>
                  </div>
                  <div className="bg-emerald-950/30 border border-emerald-900/40 rounded-lg px-4 py-3">
                    <div className="text-[11px] text-emerald-400/80 mb-1">在线点位</div>
                    <div className="text-2xl font-bold text-emerald-300">{cameraSummary.online}</div>
                  </div>
                  <div className="bg-slate-950/60 border border-slate-800 rounded-lg px-4 py-3">
                    <div className="text-[11px] text-slate-500 mb-1">离线点位</div>
                    <div className="text-2xl font-bold text-slate-200">{cameraSummary.offline}</div>
                  </div>
                  <div className="bg-cyan-950/30 border border-cyan-900/40 rounded-lg px-4 py-3">
                    <div className="text-[11px] text-cyan-400/80 mb-1">核心点位</div>
                    <div className="text-2xl font-bold text-cyan-300">{cameraSummary.core}</div>
                  </div>
                </div>
              )}
            </div>
                  ...
                  <div className="flex-1 bg-slate-900/40 border border-slate-800 rounded-xl p-5 flex flex-col overflow-hidden">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">{currentRegion.microTitle} · 当前显示 {filteredCameraPoints.length} 个点位</p>
                      </div>
                    ...
                    >
                      ...
                      <div className="absolute right-4 bottom-4 flex items-center gap-2">
                        <button type="button" className="w-9 h-9 rounded-lg bg-slate-950/90 border border-slate-700 text-slate-300 hover:text-white transition-colors">+</button>
                        <button type="button" className="w-9 h-9 rounded-lg bg-slate-950/90 border border-slate-700 text-slate-300 hover:text-white transition-colors">-</button>
                      </div>
                    </div>
                  </div>
                    ...
                    {!selectedCameraPoint ? (
                      ...
                      <div className="flex-1 flex flex-col">
                        <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 mb-4">
                          <div className="text-lg font-bold text-slate-100 mb-1">{selectedCameraPoint.name}</div>
                        ...
                        <div className="space-y-3 text-sm">
                          ...
                          <div className="flex justify-between gap-4 border-b border-slate-800 pb-3">
                            <span className="text-slate-500">战术分区</span>
                        ...
                        <div className="mt-4">
                          <div className="text-xs text-slate-500 mb-2">关联预警能力</div>
                          <div className="flex flex-wrap gap-2">
                        ...
                        <div className="mt-auto pt-5 space-y-3">
                          <button type="button" className="w-full bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-medium py-3 rounded-lg transition-colors shadow-lg shadow-cyan-600/20">
                            编辑点位信息
                          </button>
                          <div className="grid grid-cols-2 gap-3">
                            <button type="button" className="bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm py-2.5 rounded-lg border border-slate-700 transition-colors">
                              查看视频
                            </button>
                            <button type="button" className="bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm py-2.5 rounded-lg border border-slate-700 transition-colors">
                              删除点位
                ...
                <div className="flex-1 min-h-0 bg-slate-900/40 border border-slate-800 rounded-xl p-5 flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">点击行可与上方点位示意图和详情面板联动</p>
                    </div>
                      ...
                      <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-slate-950/95 backdrop-blur-sm z-10">
                          <tr className="text-left text-slate-400 border-b border-slate-800">
                            ...
                            <th className="py-3 px-3 font-medium">关联能力</th>
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="py-3 px-3 text-right whitespace-nowrap">
                                  <button type="button" className="text-cyan-300 hover:text-cyan-200 transition-colors mr-3">查看</button>
                                  <button type="button" className="text-cyan-300 hover:text-cyan-200 transition-colors mr-3">编辑</button>
                                  <button type="button" className="text-red-300 hover:text-red-200 transition-colors">删除</button>
                                </td>
                              </tr>
                            );
                          })}
        ...
        return (
          <div className="flex flex-col h-full animate-in fade-in duration-300">
            <div className="mb-6 flex items-end justify-between">
              <div>
                <h2 className="text-xl font-bold text-slate-100 mb-2">预警规则与防误报配置</h2>
                <p className="text-sm text-slate-500">为特定区域下的算法模型配置独立阈值与 ROI 电子围栏，提升实战准确率。</p>
              </div>
              <div className="flex items-center space-x-3 bg-slate-900 p-2 rounded-lg border border-slate-700">
                <span className="text-slate-400 text-xs font-medium pl-2">防区筛选：</span>
                <select 
                  ...
                  <div key={cap.id} className="bg-slate-900/60 border border-slate-700/50 rounded-xl p-5 flex flex-col shadow-sm transition-colors">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-4">
                        ...
                        <div>
                          <h3 className="text-slate-200 font-bold text-sm mb-1.5">{cap.title}</h3>
                          <p className="text-slate-400 text-xs mb-2 max-w-3xl leading-relaxed">{cap.desc}</p>
                          <div className="text-[10px] text-slate-500 mb-2">依赖感知源: {cap.sources.join(' / ')}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-5 shrink-0">
                        {cap.hasRoi && (
                          ...
                          >
                            ...
                            <span className="text-[10px] font-medium opacity-80 group-hover/roi:opacity-100 transition-opacity whitespace-nowrap">画制 ROI 围栏</span>
                          </button>
                        )}
                        <div className="flex flex-col items-center ml-2 border-l border-slate-700 pl-5">
                          ...
                          <span className={`text-[10px] font-bold ${toggles[cap.id] ? 'text-cyan-400' : 'text-slate-500'}`}>{toggles[cap.id] ? '运行中' : '已停用'}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {cap.isGathering && toggles[cap.id] && (
                      <div className="mt-5 pt-5 border-t border-slate-700/50 flex flex-col gap-4 bg-slate-950/30 -mx-5 px-5 -mb-5 pb-5 rounded-b-xl">
                        <div className="text-xs font-bold text-yellow-500 flex items-center mb-1">
                           <Settings className="w-4 h-4 mr-1.5" /> 聚集防误报双阈值调参 (参数需业务大队评估定级)
                        </div>
                        <div className="grid grid-cols-2 gap-8">
                           <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-inner">
                             <div className="flex justify-between text-xs text-slate-300 mb-4">
                               <span className="font-medium">防区内聚集人数上限</span>
                             ...
                             />
                             <div className="text-[10px] text-slate-500 mt-3 border-t border-slate-800/50 pt-2">仅当画定 ROI 内人头数超出此值时开始读秒。</div>
                           </div>
                           
                           <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-inner">
                             <div className="flex justify-between text-xs text-slate-300 mb-4">
                               <span className="font-medium">持续滞留/拥堵时长上限</span>
                             ...
                             />
                             <div className="text-[10px] text-slate-500 mt-3 border-t border-slate-800/50 pt-2">人群达标且滞留超过此时间才触发最终告警，过滤瞬间客流峰值。</div>
                           </div>
                        </div>
                      </div>
                    )}
              ...
              <div className="grid grid-cols-12 gap-0 min-h-[520px]">
                <div className="col-span-8 p-5 bg-slate-950/60 border-r border-slate-800">
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      <div className="text-sm font-bold text-slate-100">区域预览</div>
                      <div className="text-xs text-slate-500">先用预览图选择重点区域，后续可升级为拖拽画框</div>
                    </div>
                  ...
                  >
                    ...
                    <div className="absolute left-[34%] bottom-[10%] text-xs text-slate-400">可将此区域作为重点识别 ROI</div>
                  </div>
                </div>

                <div className="col-span-4 p-5 bg-slate-900/50">
                  <div className="text-sm font-bold text-slate-100 mb-4">ROI 设置</div>
                  <div className="space-y-4">
                    <div>
                      <div className="text-[11px] text-slate-500 mb-2">ROI 名称</div>
                      <input
                        value={roiName}
                    ...
                    <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4 text-sm text-slate-300 leading-relaxed">
                      当前仅提供轻量预览与阈值设置，后续可接真实拖拽画框和多 ROI 管理。
                    </div>
      ...
      return (
        <div className="flex h-full bg-[#0b1120] relative z-20">
          <div className="w-60 bg-slate-900/80 border-r border-slate-800 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.2)] shrink-0">
            ...
            <div className="flex-1 py-4 flex flex-col gap-1 px-3">
              ...
              >
                <CameraLens className="w-5 h-5 mr-3" /> 摄像头点位配置
              </button>
              <button 
              ...
              >
                <Bell className="w-5 h-5 mr-3" /> 预警规则配置
              </button>
            </div>
          </div>
          <div className="flex-1 p-6 overflow-hidden bg-gradient-to-br from-slate-950 to-[#0b1120]">
    ...
    // 历史处置记录
    // ============================================================================
    const DispositionHistoryView = ({ records, selectedRecordId, onSelectRecord, onOpenReport }) => {
          ...
          <div className="flex-1 p-6 overflow-hidden bg-gradient-to-br from-slate-950 to-[#0b1120]">
            <div className="flex flex-col h-full min-h-0 animate-in fade-in duration-300">
              <div className="mb-6 flex items-end justify-between gap-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-100 mb-2">历史处置记录</h2>
                  <p className="text-sm text-slate-500">沉淀每条告警从触发、确认、派单到解除的全过程，便于查询闭环和后续复盘。</p>
                </div>
              ...
              <div className="grid grid-cols-4 gap-3 mb-5">
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                  <div className="text-[11px] text-slate-500 mb-1">累计处置</div>
                ...
                <div className="bg-emerald-950/30 border border-emerald-900/40 rounded-xl p-4">
                  <div className="text-[11px] text-emerald-400/80 mb-1">已闭环</div>
                  <div className="text-2xl font-bold text-emerald-300">{summary.closed}</div>
                </div>
                <div className="bg-amber-950/30 border border-amber-900/40 rounded-xl p-4">
                  <div className="text-[11px] text-amber-400/80 mb-1">待复盘</div>
                  <div className="text-2xl font-bold text-amber-300">{summary.pendingReview}</div>
                </div>
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                  <div className="text-[11px] text-slate-500 mb-1">误报回流</div>
                  <div className="text-2xl font-bold text-slate-100">{summary.misreport}</div>
                </div>
              </div>

              <div className="flex-1 min-h-0 flex gap-5">
                <div className="w-[42%] min-w-[360px] bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden flex flex-col">
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start justify-between gap-3 mb-3">
                              <div>
                                <div className="text-sm font-bold text-slate-100">{stripPriorityLabel(record.alertType)}</div>
                            ...
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <span className={`text-[10px] ${record.reviewStatus === '待复盘' ? 'text-amber-400' : 'text-slate-500'}`}>{record.reviewStatus}</span>
                            </div>
                          </button>
                        );
                      })
                  ...
                  {!selectedRecord ? (
                    ...
                    <>
                      <div className="px-6 py-5 border-b border-slate-800 flex items-start justify-between gap-4">
                        <div>
                          <div className="text-[11px] text-slate-500 mb-2">{selectedRecord.id}</div>
                          <h3 className="text-lg font-bold text-slate-100">{stripPriorityLabel(selectedRecord.alertType)}</h3>
                          <p className="text-sm text-slate-400 mt-2 max-w-3xl">{selectedRecord.resultSummary}</p>
                        </div>
                        <div className="flex gap-3 shrink-0">
                          ...
                          <button type="button" className="bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm px-4 py-2.5 rounded-lg border border-slate-700 transition-colors">
                            导出处置单
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-5">
                        <div className="grid grid-cols-4 gap-3">
                          <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                            ...
                            <div className="text-sm font-semibold text-slate-100">{selectedRecord.reportedAt}</div>
                          </div>
                          <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                            <div className="text-[10px] text-slate-500 mb-1">处置时间</div>
                            <div className="text-sm font-semibold text-slate-100">{selectedRecord.handledAt}</div>
                          </div>
                          <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                            <div className="text-[10px] text-slate-500 mb-1">完成时间</div>
                            <div className="text-sm font-semibold text-slate-100">{selectedRecord.completedAt}</div>
                          </div>
                          <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                            <div className="text-[10px] text-slate-500 mb-1">现场指挥</div>
                            <div className="text-sm font-semibold text-slate-100">{selectedRecord.commander}</div>
                          </div>
                        </div>

                        <div className="grid grid-cols-3 gap-5">
                          <div className="col-span-2 bg-slate-950/40 border border-slate-800 rounded-xl p-5">
                            <div className="text-sm font-bold text-slate-200 mb-4">处置时间线</div>
                            <div className="space-y-3">
                              {selectedRecord.timeline.map(item => (
                                <div key={item} className="flex items-start">
                                  <span className="w-2.5 h-2.5 rounded-full bg-cyan-400 mt-1.5 mr-3 shrink-0"></span>
                          ...
                          <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-5">
                            <div className="text-sm font-bold text-slate-200 mb-4">证据留痕</div>
                            <div className="space-y-2">
                              {selectedRecord.evidence.map(item => (
                                <div key={item} className="px-3 py-2 rounded-lg bg-slate-900 border border-slate-800 text-sm text-slate-300">{item}</div>
                              ))}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-5">
                          <div className="text-sm font-bold text-slate-200 mb-3">复盘备注</div>
                          <div className="text-sm text-slate-300 leading-relaxed">{selectedRecord.notes}</div>
                        </div>
                      </div>
                    </>
                  )}
    ...
    // 复盘报告
    // ============================================================================
    const ReviewReportsView = ({ reports, selectedReportId, onSelectReport }) => {
      const [statusFilter, setStatusFilter] = useState('all');
        ...
        <div className="flex h-full bg-[#0b1120] relative z-20">
          <div className="flex-1 p-6 overflow-hidden bg-gradient-to-br from-slate-950 to-[#0b1120]">
            <div className="flex flex-col h-full min-h-0 animate-in fade-in duration-300">
              <div className="mb-6 flex items-end justify-between gap-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-100 mb-2">复盘报告</h2>
                  <p className="text-sm text-slate-500">针对重点事件自动沉淀复盘初稿，支持人工补充发现、策略建议和经验归档。</p>
                </div>
                <div className="flex items-center gap-3">
                  <select
                    value={statusFilter}
                  ...
                  <button type="button" className="bg-cyan-600 hover:bg-cyan-500 text-white text-sm px-4 py-2.5 rounded-lg font-medium shadow-lg shadow-cyan-600/20 transition-colors">
                    导出复盘 PDF
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 min-h-0 flex gap-5">
                <div className="w-[38%] min-w-[340px] bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden flex flex-col">
                  <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
                    <h3 className="text-sm font-bold text-slate-200">报告清单</h3>
                    <span className="text-[10px] text-slate-500">{filteredReports.length} 份</span>
                  </div>
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3 mb-2">
                            <div className="text-sm font-bold text-slate-100 leading-relaxed">{report.title}</div>
                            <span className={`px-2 py-1 rounded-md text-[10px] border ${report.status === '待复盘' ? 'bg-amber-950/30 text-amber-300 border-amber-800/40' : 'bg-emerald-950/30 text-emerald-300 border-emerald-800/40'}`}>
                              {report.status}
                            </span>
                          </div>
                          <div className="text-[11px] text-slate-500 mb-1">{report.location}</div>
                ...
                <div className="flex-1 min-w-0 bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden flex flex-col">
                  {!selectedReport ? (
                    <div className="h-full flex items-center justify-center text-slate-500">请选择左侧复盘报告</div>
                  ) : (
                    <>
                      <div className="px-6 py-5 border-b border-slate-800">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <div className="text-[11px] text-slate-500 mb-2">{selectedReport.id} / {selectedReport.stageNote}</div>
                            <h3 className="text-lg font-bold text-slate-100">{selectedReport.title}</h3>
                            <p className="text-sm text-slate-400 mt-2 max-w-3xl">{selectedReport.summary}</p>
                          </div>
                          <div className="text-right shrink-0">
                            <div className="text-[11px] text-slate-500">事件窗口</div>
                            <div className="text-sm font-semibold text-slate-100 mt-1">{selectedReport.eventWindow}</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-5">
                        <div className="grid grid-cols-4 gap-3">
                          <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                            <div className="text-[10px] text-slate-500 mb-1">事件类型</div>
                            <div className="text-sm font-semibold text-slate-100">{stripPriorityLabel(selectedReport.type)}</div>
                          </div>
                          <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                            <div className="text-[10px] text-slate-500 mb-1">地点</div>
                            <div className="text-sm font-semibold text-slate-100">{selectedReport.location}</div>
                          </div>
                          <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                            <div className="text-[10px] text-slate-500 mb-1">责任单位</div>
                            <div className="text-sm font-semibold text-slate-100">{selectedReport.owner}</div>
                          </div>
                          <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                            <div className="text-[10px] text-slate-500 mb-1">严重等级</div>
                        <div className="text-sm font-semibold text-red-300">{severityText(selectedReport.severity)}</div>
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-5">
                          <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-5">
                            <div className="text-sm font-bold text-slate-200 mb-4">关键发现</div>
                            <div className="space-y-3">
                              {selectedReport.keyFindings.map(item => (
                                <div key={item} className="flex items-start">
                                  <span className="w-2.5 h-2.5 rounded-full bg-red-400 mt-1.5 mr-3 shrink-0"></span>
                                  <span className="text-sm text-slate-300 leading-relaxed">{item}</span>
                                </div>
                              ))}
                            </div>
                          ...
                          <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-5">
                            <div className="text-sm font-bold text-slate-200 mb-4">事件时间线</div>
                            <div className="space-y-3">
                        ...
                        <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-5">
                          <div className="text-sm font-bold text-slate-200 mb-4">优化建议</div>
                          <div className="grid grid-cols-3 gap-4">
                            {selectedReport.recommendations.map(item => (
                              <div key={item} className="bg-slate-900/70 border border-slate-800 rounded-lg p-4 text-sm text-slate-300 leading-relaxed">
                                {item}
                              </div>
                            ))}
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-5">
                          <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-5">
                            <div className="text-sm font-bold text-slate-200 mb-4">关键词</div>
                            <div className="flex flex-wrap gap-2">
                              {selectedReport.keywords.map(item => (
                                <span key={item} className="px-3 py-2 rounded-lg bg-cyan-950/30 border border-cyan-800/40 text-cyan-200 text-sm">{item}</span>
                              ))}
                            </div>
                          </div>
                          <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-5">
                            <div className="text-sm font-bold text-slate-200 mb-4">复盘结论</div>
                            <div className="text-sm text-slate-300 leading-relaxed">{selectedReport.conclusion}</div>
                          </div>
                        </div>
                      </div>
                    </>
    ...
    // 监控巡看
    // ============================================================================
    const SurveillanceView = ({ activeRegions, alerts, onAlertClick }) => {
      ...
      return (
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {activeRegions.map(id => (
                    <option key={id} value={id}>{ALL_REGIONS_CONFIG[id]?.name || id}</option>
                  ))}
                </select>
              </div>
              <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-4">
                <div className="grid grid-cols-3 gap-3 shrink-0">
                  <div className="bg-slate-900/40 border border-slate-800 rounded-xl px-4 py-3">
                    ...
                    <div className="text-2xl font-bold text-emerald-300">{filteredFeeds.filter(feed => feed.status === 'online').length}</div>
                  </div>
                  <div className="bg-slate-900/40 border border-slate-800 rounded-xl px-4 py-3">
                    <div className="text-[11px] text-slate-500 mb-1">当前未处置告警</div>
                    <div className="text-2xl font-bold text-red-400">{unhandledAlerts.length}</div>
                  </div>
                  <div className="bg-slate-900/40 border border-slate-800 rounded-xl px-4 py-3">
              ...
              <div className="w-[320px] shrink-0 bg-slate-900/40 border border-slate-800 rounded-xl p-5 flex flex-col">
                ...
                {selectedFeed ? (
                  <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-1">
                    ...
                    <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                      <div className="text-[10px] text-slate-500 mb-2">值守标签</div>
                      <div className="flex flex-wrap gap-2">
                        {selectedFeed.tags.map(tag => (
                          <span key={tag} className="px-2.5 py-1 rounded-md bg-cyan-950/30 border border-cyan-800/40 text-cyan-200 text-[11px]">
                            {tag}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                      <div className="text-[10px] text-slate-500 mb-2">巡看摘要</div>
                      <div className="text-sm text-slate-300 leading-relaxed">{selectedFeed.summary}</div>
                    </div>
        ...
        {expandedFeed && (
          <div className="absolute inset-0 z-[70] bg-slate-950/88 backdrop-blur-sm flex items-center justify-center p-6">
            <div className="w-full max-w-6xl rounded-2xl border border-slate-700 bg-slate-900 shadow-[0_0_60px_rgba(2,6,23,0.65)] overflow-hidden">
              ...
              <div className="p-5 grid grid-cols-[minmax(0,1fr)_320px] gap-5">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                    <div className="text-[10px] text-slate-500 mb-2">巡看摘要</div>
                    <div className="text-sm text-slate-300 leading-relaxed">{expandedFeed.summary}</div>
                  </div>
    ...
    function App() {
      ...
      const buildDispositionRecordFromAlert = (alert, newStatus) => {
        ...
        return {
          ...
          tags: newStatus === 'dispatched'
            ? [alert.severity, '自动生成', isGathering ? '聚集场景' : '重点告警']
            : newStatus === 'reviewing'
      ...
      return (
        <div className="h-screen w-full bg-[#0f172a] text-slate-200 font-sans flex overflow-hidden">
          <nav className="w-16 bg-[#0f172a] border-r border-slate-800 flex flex-col items-center py-4 shrink-0 z-40 relative shadow-[4px_0_24px_rgba(0,0,0,0.5)]">
            <div className="bg-red-900/40 p-1.5 rounded-lg border border-red-500/50 mb-8 shadow-[0_0_15px_rgba(239,68,68,0.4)]"><Shield className="w-5 h-5 text-red-400" /></div>
            <div className="flex-1 flex flex-col gap-4 w-full px-2">
              <button onClick={() => setCurrentView('dashboard')} className={`w-full aspect-square flex flex-col items-center justify-center rounded-lg transition-all ${currentView === 'dashboard' ? 'bg-cyan-900/40 text-cyan-400 border border-cyan-500/50 shadow-inner' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`}><LayoutDashboard className="w-5 h-5 mb-1" /><span className="text-[9px]">大屏态势</span></button>
              <button onClick={() => setCurrentView('surveillance')} className={`w-full aspect-square flex flex-col items-center justify-center rounded-lg transition-all ${currentView === 'surveillance' ? 'bg-cyan-900/40 text-cyan-400 border border-cyan-500/50 shadow-inner' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`}><Video className="w-5 h-5 mb-1" /><span className="text-[9px]">监控巡看</span></button>
              <button onClick={() => setCurrentView('records')} className={`w-full aspect-square flex flex-col items-center justify-center rounded-lg transition-all ${currentView === 'records' ? 'bg-cyan-900/40 text-cyan-400 border border-cyan-500/50 shadow-inner' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`}><ListCheck className="w-5 h-5 mb-1" /><span className="text-[9px]">处置记录</span></button>
              <button onClick={() => setCurrentView('reports')} className={`w-full aspect-square flex flex-col items-center justify-center rounded-lg transition-all ${currentView === 'reports' ? 'bg-cyan-900/40 text-cyan-400 border border-cyan-500/50 shadow-inner' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`}><BarChart3 className="w-5 h-5 mb-1" /><span className="text-[9px]">复盘报告</span></button>
              <button onClick={() => setCurrentView('config')} className={`w-full aspect-square flex flex-col items-center justify-center rounded-lg transition-all mt-auto ${currentView === 'config' ? 'bg-cyan-900/40 text-cyan-400 border border-cyan-500/50 shadow-inner' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`}><Settings className="w-5 h-5 mb-1" /><span className="text-[9px]">系统配置</span></button>
            </div>
          </nav>
          
          <div className="flex-1 flex flex-col overflow-hidden relative">
            {(currentView === 'dashboard' || currentView === 'surveillance' || currentView === 'records' || currentView === 'reports') && (
              <header className="h-14 border-b border-slate-800/80 bg-[#0f172a]/90 backdrop-blur-sm flex items-center justify-between px-6 z-10 shrink-0">
                <h1 className="text-lg font-bold flex items-center tracking-widest drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                   <span className="text-red-400">翔安区社会面治安防控智能感知系统</span>
                   <span className="text-xs text-cyan-500/70 ml-4 font-normal tracking-normal border border-cyan-800/50 bg-cyan-950/40 px-2 py-0.5 rounded backdrop-blur-sm">
                     {currentView === 'dashboard' ? `${activeRegions.length} 重点防区运行中` : currentView === 'surveillance' ? `${SURVEILLANCE_FEEDS.filter(feed => activeRegions.includes(feed.regionId) && feed.status === 'online').length} 路在线巡看` : currentView === 'records' ? `${dispositionRecords.length} 条处置记录` : `${reviewReports.length} 份复盘报告`}
                   </span>
                </h1>
                <div className="text-red-400 font-mono text-base tracking-widest flex items-center drop-shadow-md">
                  <span className="w-2 h-2 bg-red-500 rounded-full mr-3 animate-ping shadow-[0_0_5px_red]"></span>
            ...
            <main className={`flex-1 overflow-hidden relative ${currentView === 'dashboard' ? 'p-5' : ''}`}>
              {currentView === 'dashboard' && <DashboardView activeRegions={activeRegions} alerts={alerts} onAlertClick={setActiveAlert} />}
              {currentView === 'surveillance' && <SurveillanceView activeRegions={activeRegions} alerts={alerts} onAlertClick={setActiveAlert} />}
              {currentView === 'records' && (
              ...
              {currentView === 'config' && <SystemConfigView activeRegions={activeRegions} setActiveRegions={setActiveRegions} />}
              <AlertVerificationModal alert={activeAlert} onClose={() => setActiveAlert(null)} onResolve={handleResolveAlert} />
            </main>
          </div>
        </div>
      );
